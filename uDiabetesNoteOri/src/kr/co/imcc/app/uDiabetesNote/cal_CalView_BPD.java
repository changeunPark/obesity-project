...
public class cal_CalView_BPD extends View {
	...
	public cal_CalView_BPD(Context context) {
		super(context);
...
//		invalidate(selRect);
	    ...
	    sqlDataCheck = "select * from BlOODPRESSUREBPD where date=";
	    
	    if(!SELECT_FLAG)
	    {
	    	if((ActivityBloodPressure.sltDate[0] != calendarCheck.get(Calendar.YEAR) || 
	    			ActivityBloodPressure.sltDate[1] != calendarCheck.get(Calendar.MONTH))) 
	    	{
    			SELECT_FLAG = true;
	    		
	    		getRect(ActivityBloodPressure.iDate[0] , 0, selRect);
