...
public class cal_CalView_hb extends View {
	...
	private final Activity1_hb Cal;
	public cal_CalView_hb(Context context) {
		super(context);
		this.Cal = (Activity1_hb) context;
		setFocusable(true);
...
//		int nam=0;
//		if( (Activity1_hb.iDate[0] +Activity1_hb.iDate[1]) % 7 != 0)
//			nam = 1;
//		column = ((Activity1_hb.iDate[0] +Activity1_hb.iDate[1]) / 7 ) +nam;
//		
		width = w / 7f;
...
//		invalidate(selRect);
		...
		change = ((Activity1_hb.iDate[0] +Activity1_hb.iDate[1]) / 7 ) +nam;
	    ...
	    sqlDataCheck = "select * from HBA1C where date=";
	    
	    if(!SELECT_FLAG)
	    {
	    	if((Activity1_hb.sltDate[0] != calendarCheck.get(Calendar.YEAR) || 
	    			Activity1_hb.sltDate[1] != calendarCheck.get(Calendar.MONTH))) 
	    	{
	    		SELECT_FLAG = true;
	    		
	    		getRect(Activity1_hb.iDate[0] , 0, selRect);	    
				...
				Activity1_hb.SelectedDate2 = Cal.getDateYear()+"/"+month+"/"+day;
				Activity1_hb.SelectedDate = Cal.getDateYear()+"�� "+Cal.getDateMonth()+"�� "+dayCounut+"��";
				
				Cal.showHbList(Activity1_hb.SelectedDate2);
	    	}	  
	    }
	    ...
	    for( int b = 0; b < Activity1_hb.iDate[1]; b++){
	    	String numTemp = Integer.toString(dayCounut);
	    	
	    	if((Activity1_hb.iDate[0] + dayCounut) % 7 == 0){
	    		foreground.setColor(Color.BLUE);
	    	}
	    	else if((Activity1_hb.iDate[0] + dayCounut) % 7 == 1){
	    		foreground.setColor(Color.RED);
	    	}
	    	else {
	    		foreground.setColor(getTextColor());
	    	...
	    	// ������ ��¥ ǥ���ϱ�
	    	if( (Activity1_hb.sltDate[0] == calendarCheck.get(Calendar.YEAR) && 
	    			Activity1_hb.sltDate[1] == calendarCheck.get(Calendar.MONTH) && Activity1_hb.iDate[2] == dayCounut) ) {
	    		foreground.setTextSize(gettextSize(height));
	    	...
	    	//Log.d("date111111111111111111", Cal.getDateYear()+"�� "+Cal.getDateMonth()+"�� " +numTemp + "�� " + Activity1_hb.iDate[1]);
	    	//db
			...
			cursorDataCheck = Activity1_hb.sqLiteDatabase.rawQuery(sqlDataCheck + "'" + Cal.getDateYear()+"/"+month+"/" +day + "'", null);
	    	...
	    	if(cursorDataCheck.getCount()>0){
	    		...
	    		canvas.drawBitmap(bit,x + width * col + 18, y + penYValue ,null);	    		*/
	    	}
	    	...
	    	// ����
	    	String tempy = toString(Activity1_hb.sltDate[0]);
	    	String tempm = toString(Activity1_hb.sltDate[1]);
	    	String tempd = "0";
	    	if ( numTemp.length() == 1)
	    		tempd += numTemp;
	    	else
...
//			Log.d("day", cal[(int) (event.getX() / width)][(int) (((event.getY()-height2) / height))] + "");
			...
			if(!day.equals("00")){
				...
				Activity1_hb.calSelectColor = getResources().getColor(R.color.cal_selected);
				Activity1_hb.calendarView.setSelectColor(Activity1_hb.calSelectColor);	
				Activity1_hb.calendarView.invalidate();
				
				Activity1_hb.SelectedDate = Cal.getDateYear()+"�� "+Cal.getDateMonth()+"�� "+cal[(int) (event.getX() / width)][(int) (((event.getY()-height2) / height))]+"��";
